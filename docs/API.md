...
**Output** (`data` on success or error):

| Field | Type | Description |
|-------|------|-------------|
| `cmd` | `string` | Echoed command |
| `stdout` | `string` | Bounded stdout (10 MB) |
| `stderr` | `string` | Bounded stderr (10 MB) |
...
> `timed_out` replaces the prior `code().is_none()` inference (RC3 fix) — the only timeout sentinel.

**Example:**
```bash
runtimo run -c ShellExec -a '{"cmd":"echo hello && whoami"}'
# → {"cmd":"echo hello && whoami","exit_code":0,"stdout":"hello\nuser\n","timed_out":false,"signal":null,...}
# timeout case → {"exit_code":-1,"timed_out":true,"signal":9,"stderr":"","stdout":"...partial..."}
```
...
### runtimo observe (daemon JSON-RPC)

Observe collects process samples into a WAL-backed bundle (`data_dir/bundles/<run_id>.jsonl`).
